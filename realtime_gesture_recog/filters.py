...
def greyMask(frame, x0, y0, width, height):
    roi = frame[y0:y0 + height, x0:x0 + width]
    res = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
...
def skinMask(frame, x0, y0, width, height):
    ...
    # low_range = np.array([0, 20, 70])
    # upper_range = np.array([50, 180, 240])
...
if __name__ == "__main__":
    img = cv2.imread(r'test_img/3.jpg')
    np_img = np.array(img)

    from PIL import Image

    img = skinMask(np_img,250,200,400,400)
